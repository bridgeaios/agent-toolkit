...
- Max function size: 200MB (including dependencies)
...
- 200 invocations per function per 30 seconds
- 4000 invocations per project per 30 seconds

### Cost

Cost = invocations × (memory GB × duration seconds). Default is 1GB memory. A function averaging 1GB and 40ms duration can run ~500k invocations within 20K GB-seconds.
